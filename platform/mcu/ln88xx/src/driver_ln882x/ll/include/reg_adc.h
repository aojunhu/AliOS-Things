...
__STATIC_INLINE void adc_eoc_setf(uint32_t base, uint8_t eoc)
{
    ((t_hwp_adc_t*)(base))->adc_isr.bit_field.eoc = eoc;
}
...
__STATIC_INLINE uint8_t adc_eoc_getf(uint32_t base)
{
    return ((t_hwp_adc_t*)(base))->adc_isr.bit_field.eoc;
}
...
__STATIC_INLINE void adc_isr_unpack(uint32_t base, uint8_t * awd, uint8_t * ovr, uint8_t * eos, uint8_t * eoc)
{
    t_adc_isr local_val = ((t_hwp_adc_t*)(base))->adc_isr;

    *awd                = local_val.bit_field.awd;
    *ovr                = local_val.bit_field.ovr;
    *eos                = local_val.bit_field.eos;
    *eoc                = local_val.bit_field.eoc;
}

__STATIC_INLINE void adc_ier_unpack(uint32_t base, uint8_t * awdie, uint8_t * ovrie, uint8_t * eosie, uint8_t * eocie)
{
    t_adc_ier local_val = ((t_hwp_adc_t*)(base))->adc_ier;

    *awdie              = local_val.bit_field.awdie;
    *ovrie              = local_val.bit_field.ovrie;
    *eosie              = local_val.bit_field.eosie;
    *eocie              = local_val.bit_field.eocie;
}

__STATIC_INLINE void adc_cr_unpack(uint32_t base, uint8_t * adstp, uint8_t * adstart, uint8_t * aden)
{
    t_adc_cr local_val = ((t_hwp_adc_t*)(base))->adc_cr;

    *adstp              = local_val.bit_field.adstp;
    *adstart            = local_val.bit_field.adstart;
    *aden               = local_val.bit_field.aden;
}

__STATIC_INLINE void adc_cfgr1_unpack(uint32_t base, uint8_t * awdch, uint8_t * awden, uint8_t * awdsgl, uint8_t * autoff, uint8_t * waitm, uint8_t * cont, uint8_t * align, uint8_t * spec_exten, uint8_t * spec_extsel, uint8_t * exten, uint8_t * extsel)
{
    t_adc_cfgr1 local_val = ((t_hwp_adc_t*)(base))->adc_cfgr1;

    *awdch              = local_val.bit_field.awdch;
    *awden              = local_val.bit_field.awden;
    *awdsgl             = local_val.bit_field.awdsgl;
    *autoff             = local_val.bit_field.autoff;
    *waitm              = local_val.bit_field.waitm;
    *cont               = local_val.bit_field.cont;
    *align              = local_val.bit_field.align;
    *spec_exten         = local_val.bit_field.spec_exten;
    *spec_extsel        = local_val.bit_field.spec_extsel;
    *exten              = local_val.bit_field.exten;
    *extsel             = local_val.bit_field.extsel;
}

__STATIC_INLINE void adc_cfgr2_unpack(uint32_t base, uint8_t * ibuf_b, uint8_t * vset, uint8_t * cal_en, uint8_t * stab, uint8_t * vref_sel, uint8_t * mode_sel, uint8_t * ovsr, uint8_t * ovse)
{
    t_adc_cfgr2 local_val = ((t_hwp_adc_t*)(base))->adc_cfgr2;

    *ibuf_b             = local_val.bit_field.ibuf_b;
    *vset               = local_val.bit_field.vset;
    *cal_en             = local_val.bit_field.cal_en;
    *stab               = local_val.bit_field.stab;
    *vref_sel           = local_val.bit_field.vref_sel;
    *mode_sel           = local_val.bit_field.mode_sel;
    *ovsr               = local_val.bit_field.ovsr;
    *ovse               = local_val.bit_field.ovse;
}

__STATIC_INLINE void adc_tr_unpack(uint32_t base, uint16_t * ht, uint16_t * lt)
{
    t_adc_tr local_val = ((t_hwp_adc_t*)(base))->adc_tr;

    *ht                 = local_val.bit_field.ht;
    *lt                 = local_val.bit_field.lt;
}

__STATIC_INLINE void adc_chselr_unpack(uint32_t base, uint8_t * spec_sw_start, uint8_t * spec_ch, uint8_t * chselx)
{
    t_adc_chselr local_val = ((t_hwp_adc_t*)(base))->adc_chselr;

    *spec_sw_start      = local_val.bit_field.spec_sw_start;
    *spec_ch            = local_val.bit_field.spec_ch;
    *chselx             = local_val.bit_field.chselx;
}

__STATIC_INLINE void adc_ccr_unpack(uint32_t base, uint8_t * auxadc_ckinv, uint8_t * presc)
{
    t_adc_ccr local_val = ((t_hwp_adc_t*)(base))->adc_ccr;

    *auxadc_ckinv       = local_val.bit_field.auxadc_ckinv;
    *presc              = local_val.bit_field.presc;
}
