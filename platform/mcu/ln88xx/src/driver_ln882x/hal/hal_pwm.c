...
/**
 * @brief Set clock divsion of one channel.
 *
 * @param chan
 * @param div clock divsion, 0--1, 1--2, ..., 63--64.
 */
void HAL_PWM_Div_Set(pwm_channel_enum_t chan, uint8_t div)
{
    assert_param(IS_PWM_CHAN(chan));
...
void HAL_PWM_Config(pwm_channel_enum_t chan, uint32_t freq, float duty)
{
    ...
    uint8_t div = LL_PWM_Div_Get(chan);
    uint32_t realfreq = APBUS0_CLOCK / (div+1);
