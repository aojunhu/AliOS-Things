...
void ll_rf_art2000_sys_awo_init(void)
{
    ...
    sysc_awo_pmu_reg_set(0x81042086); //  (BUCK)  r_vtrim: bit[9:8]=0b00 //00: 1.232 / 01: 1.216  / 10: 1.201 / 11: 1.185
    sysc_awo_pmu_cfg_set(0x20006003); //  o_digldo_norvdd: bit[10:8]=0b000  0x00->1.1V/ 0x01->1.05V/ 0x02->1.0V/ 0x03->0.95V/ 0x04->0.90V/ 0x05->0.85V/ 0x06->0.80V/ 0x07->1.2V
...
{
    // rf 0x0d, bit10,bit9 set to 1.
    ll_rf_art2000_reg_write(0, 0x01, 0x0010); // rf_auto_cal
}
...
{
    ll_rf_art2000_reg_write(0, 0x01, 0x0000); // disable rf_auto_cal 
}
