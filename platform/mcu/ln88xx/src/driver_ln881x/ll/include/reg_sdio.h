...
typedef union
{
    ...
    struct
    {
        ...
        uint32_t               support_func_num :  3; /* 6: 4, support function number available 0~7, will be filled in R4 for CMD5*/
...
typedef union
{
    ...
    struct
    {
        ...
        uint32_t                        iir_reg :  1; /*24:24, CPU can write this register to set DAT1 interrupt; CMD52 can set this register by write SDIO_HOST_IIR_REG*/
...
typedef union
{
    ...
    struct
    {
        uint32_t                     status_reg : 12; /*11: 0, write 0 to clear these registers, these intrs will set only when enable is set.
...
bit 11: fn1_int_en_up assert when CMD52 write FN1 addr 8'h14 to 1*/
        ...
        uint32_t                    fn1_m2s_int :  1; /*20:20, bit 20 fn1_m2s_int including m2s1, m2s2, m2s3, m2s4 interrupt*/
        uint32_t                     reserved_0 : 11; /*31:21,                             NA*/
    } bit_field;
...
typedef struct
{
    ...
    volatile             T_SDIO_SDIO_PROGREG                   sdio_progreg; /*  0x8,    RW, 0x00000076,                             NA*/
    ...
    volatile            T_SDIO_SDIO_FN1_SPEC                  sdio_fn1_spec; /*0x110,    RW, 0x00000000,                             NA*/
    ...
    volatile        T_SDIO_SDIO_AHB_INT_STS1              sdio_ahb_int_sts1; /*0x134,    RW, 0x00000000,                             NA*/
...
#define hwp_sdio ((T_HWP_SDIO_T*)REG_SDIO_BASE)
...
__STATIC_INLINE void sdio_sdio_progreg_pack(uint8_t support_func_num, uint8_t cpu_in_active, uint8_t dev_ready)
{
    hwp_sdio->sdio_progreg.val = (((uint32_t)support_func_num << 4) | ((uint32_t)cpu_in_active << 2) | ((uint32_t)dev_ready << 0));
}

__STATIC_INLINE void sdio_sdio_progreg_unpack(uint8_t* support_func_num, uint8_t* cpu_in_active, uint8_t* card_ready, uint8_t* dev_ready)
{
    T_SDIO_SDIO_PROGREG localVal = hwp_sdio->sdio_progreg;

    *support_func_num = localVal.bit_field.support_func_num;
...
__STATIC_INLINE uint8_t sdio_support_func_num_getf(void)
{
    return hwp_sdio->sdio_progreg.bit_field.support_func_num;
}

__STATIC_INLINE void sdio_support_func_num_setf(uint8_t support_func_num)
{
    hwp_sdio->sdio_progreg.bit_field.support_func_num = support_func_num;
}
...
__STATIC_INLINE void sdio_sdio_fn1_spec_set(uint32_t value)
{
    hwp_sdio->sdio_fn1_spec.val = value;
}
...
__STATIC_INLINE void sdio_sdio_fn1_spec_unpack(uint8_t* retry_ctrl, uint8_t* wr_pkt_ctrl, uint8_t* rd_pkt_ctrl)
{
...
__STATIC_INLINE void sdio_sdio_ahb_int_sts1_pack(uint8_t fn1_m2s_int, uint16_t status_reg)
{
    hwp_sdio->sdio_ahb_int_sts1.val = (((uint32_t)fn1_m2s_int << 20) | ((uint32_t)status_reg << 0));
}
