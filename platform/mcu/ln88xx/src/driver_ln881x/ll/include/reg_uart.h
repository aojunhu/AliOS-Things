...
NOTE: If a FIFO is full when a break condition is received, a FIFO overrun occurs. The break condition and all the information associated with it¡ªparity and framing errors¡ªis discarded; any information that a break character was received is lost.*/
...
typedef union
{
    ...
    struct
    {
        uint32_t                          lpdll :  8; /* 7: 0, This register makes up the lower 8-bits of a 16-bit, read/write, Low Power Divisor Latch register that contains the baud rate divisor for the UART, which must give a baud rate of 115.2K. This is required for SIR Low Power (minimum pulse width) detection at the receiver.
...
typedef union
{
    ...
    struct
    {
        uint32_t                          lpdlh :  8; /* 7: 0, This register makes up the upper 8-bits of a 16-bit, read/write, Low Power Divisor Latch register that contains the baud rate divisor for the UART, which must give a baud rate of 115.2K. This is required for SIR Low Power (minimum pulse width) detection at the receiver.
                                                         The output low-power baud rate is equal to the serial clock (sclk) frequency divided by sixteen times the value of the baud rate divisor, as follows:
                                                         Low power baud rate = (serial clock frequency)/(16* divisor)
                                                         Therefore, a divisor must be selected to give a baud rate of 115.2K.
...
typedef union
{
    ...
    struct
    {
        uint32_t                            dlf :  4; /* 3: 0, uart float part of baud dividor*/
...
typedef union
{
    ...
    struct
    {
        uint32_t                 apb_data_width :  2; /* 1: 0, 00 - 8 bits
                                                         01 - 16 bits
                                                         10 - 32 bits
                                                         11 - reserved*/
        uint32_t                     reserved_2 :  2; /* 3: 2,                             NA*/
        uint32_t                      afce_mode :  1; /* 4: 4,           0 - FALSE
                                                         1 - TRUE*/
        uint32_t                      thre_mode :  1; /* 5: 5,           0 - FALSE
                                                         1 - TRUE*/
        uint32_t                       sir_mode :  1; /* 6: 6,           0 - FALSE
                                                         1 - TRUE*/
        uint32_t                    sir_lp_mode :  1; /* 7: 7,           0 - FALSE
                                                         1 - TRUE*/
        uint32_t                       new_feat :  1; /* 8: 8,           0 - FALSE
                                                         1 - TRUE*/
        uint32_t                    fifo_access :  1; /* 9: 9,           0 - FALSE
                                                         1 - TRUE*/
        uint32_t                      fifo_stat :  1; /*10:10,           0 - FALSE
                                                        1 - TRUE*/
        uint32_t                         shadow :  1; /*11:11,           0 - FALSE
                                                        1 - TRUE*/
        uint32_t        uart_add_encoded_params :  1; /*12:12,           0 - FALSE
                                                        1 - TRUE*/
        uint32_t                      dma_extra :  1; /*13:13,           0 - FALSE
                                                        1 - TRUE*/
...
typedef struct
{
    ...
    volatile                   T_UART_LPDLL                          lpdll; /* 0x20,    RW, 0x00000000,  Low Power Divisor Latch (Low)*/
    volatile                   T_UART_LPDLH                          lpdlh; /* 0x24,    RW, 0x00000000, Low Power Divisor Latch (High)*/
    ...
    volatile                     T_UART_RFL                            rfl; /* 0x84,    RO, 0x00000006,                    UART Status*/
    volatile                        uint32_t                  reserved_2[7];
    ...
    volatile                     T_UART_CPR                            cpr; /* 0xf4,    RO, 0x000135F2,            Component Parameter*/
...
__STATIC_INLINE void uart_uart_en(T_HWP_UART_T *hw_uart, uint8_t enable)
{
    hw_uart->mcr.bit_field.loopback = ~enable;
}


__STATIC_INLINE uint32_t uart_rbr_get(T_HWP_UART_T *hw_uart)
{

    return hw_uart->rbr_thr_dll.rbr.val;
}

__STATIC_INLINE void uart_rbr_unpack(T_HWP_UART_T *hw_uart, uint8_t* rbr_msb_9thbit, uint8_t* rbr_lsb_8bits)
{
    T_UART_RBR localVal;

    localVal = hw_uart->rbr_thr_dll.rbr;
    *rbr_msb_9thbit = localVal.bit_field.rbr_msb_9thbit;
    *rbr_lsb_8bits = localVal.bit_field.rbr_lsb_8bits;
}

__STATIC_INLINE uint8_t uart_rbr_msb_9thbit_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->rbr_thr_dll.rbr.bit_field.rbr_msb_9thbit;
}

__STATIC_INLINE uint8_t uart_rbr_lsb_8bits_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->rbr_thr_dll.rbr.bit_field.rbr_lsb_8bits;
}

__STATIC_INLINE uint32_t uart_thr_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->rbr_thr_dll.thr.val;
}

__STATIC_INLINE void uart_thr_set(T_HWP_UART_T *hw_uart, uint32_t value)
{
    hw_uart->rbr_thr_dll.thr.val = value;
}

__STATIC_INLINE void uart_thr_pack(T_HWP_UART_T *hw_uart, uint8_t thr_msb_9thbit, uint8_t thr_lsb_8bits)
{
    hw_uart->rbr_thr_dll.thr.val = (((uint32_t)thr_msb_9thbit << 8) | ((uint32_t)thr_lsb_8bits << 0));
}

__STATIC_INLINE uint32_t uart_dll_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->rbr_thr_dll.dll.val;
}

__STATIC_INLINE void uart_dll_set(T_HWP_UART_T *hw_uart, uint32_t value)
{
    hw_uart->rbr_thr_dll.dll.val = value;
}

__STATIC_INLINE void uart_dll_pack(T_HWP_UART_T *hw_uart, uint8_t divisor_latch_low)
{
    hw_uart->rbr_thr_dll.dll.val = (((uint32_t)divisor_latch_low << 0));
}

__STATIC_INLINE void uart_dll_unpack(T_HWP_UART_T *hw_uart, uint8_t* divisor_latch_low)
{
    T_UART_DLL localVal;

    localVal = hw_uart->rbr_thr_dll.dll;
    *divisor_latch_low = localVal.bit_field.divisor_latch_low;
}

__STATIC_INLINE uint8_t uart_divisor_latch_low_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->rbr_thr_dll.dll.bit_field.divisor_latch_low;
}

__STATIC_INLINE void uart_divisor_latch_low_setf(T_HWP_UART_T *hw_uart, uint8_t divisor_latch_low)
{
    hw_uart->rbr_thr_dll.dll.bit_field.divisor_latch_low = divisor_latch_low;
}

__STATIC_INLINE uint32_t uart_dlh_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->dlh_ier.dlh.val;
}

__STATIC_INLINE void uart_dlh_set(T_HWP_UART_T *hw_uart, uint32_t value)
{
    hw_uart->dlh_ier.dlh.val = value;
}

__STATIC_INLINE void uart_dlh_pack(T_HWP_UART_T *hw_uart, uint8_t divisor_latch_high)
{
    hw_uart->dlh_ier.dlh.val = (((uint32_t)divisor_latch_high << 0));
}

__STATIC_INLINE void uart_dlh_unpack(T_HWP_UART_T *hw_uart, uint8_t* divisor_latch_high)
{
    T_UART_DLH localVal;

    localVal = hw_uart->dlh_ier.dlh;
    *divisor_latch_high = localVal.bit_field.divisor_latch_high;
}

__STATIC_INLINE uint8_t uart_divisor_latch_high_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->dlh_ier.dlh.bit_field.divisor_latch_high;
}

__STATIC_INLINE void uart_divisor_latch_high_setf(T_HWP_UART_T *hw_uart, uint8_t divisor_latch_high)
{
    hw_uart->dlh_ier.dlh.bit_field.divisor_latch_high = divisor_latch_high;
}

__STATIC_INLINE uint32_t uart_ier_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->dlh_ier.ier.val;
}

__STATIC_INLINE void uart_ier_set(T_HWP_UART_T *hw_uart, uint32_t value)
{
    hw_uart->dlh_ier.ier.val = value;
}

__STATIC_INLINE void uart_ier_pack(T_HWP_UART_T *hw_uart, uint8_t ptime, uint8_t edssi, uint8_t elsi, uint8_t etbei, uint8_t erbfi)
{
    hw_uart->dlh_ier.ier.val = (((uint32_t)ptime << 7) | ((uint32_t)edssi << 3) | ((uint32_t)elsi << 2) | ((uint32_t)etbei << 1) | ((uint32_t)erbfi << 0));
}

__STATIC_INLINE void uart_ier_unpack(T_HWP_UART_T *hw_uart, uint8_t* ptime, uint8_t* edssi, uint8_t* elsi, uint8_t* etbei, uint8_t* erbfi)
{
    T_UART_IER localVal;

    localVal = hw_uart->dlh_ier.ier;
...
__STATIC_INLINE uint8_t uart_ptime_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->dlh_ier.ier.bit_field.ptime;
}

__STATIC_INLINE void uart_ptime_setf(T_HWP_UART_T *hw_uart, uint8_t ptime)
{
    hw_uart->dlh_ier.ier.bit_field.ptime = ptime;
}

__STATIC_INLINE uint8_t uart_edssi_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->dlh_ier.ier.bit_field.edssi;
}

__STATIC_INLINE void uart_edssi_setf(T_HWP_UART_T *hw_uart, uint8_t edssi)
{
    hw_uart->dlh_ier.ier.bit_field.edssi = edssi;
}

__STATIC_INLINE uint8_t uart_elsi_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->dlh_ier.ier.bit_field.elsi;
}

__STATIC_INLINE void uart_elsi_setf(T_HWP_UART_T *hw_uart, uint8_t elsi)
{
    hw_uart->dlh_ier.ier.bit_field.elsi = elsi;
}

__STATIC_INLINE uint8_t uart_etbei_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->dlh_ier.ier.bit_field.etbei;
}

__STATIC_INLINE void uart_etbei_setf(T_HWP_UART_T *hw_uart, uint8_t etbei)
{
    hw_uart->dlh_ier.ier.bit_field.etbei = etbei;
}

__STATIC_INLINE uint8_t uart_erbfi_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->dlh_ier.ier.bit_field.erbfi;
}

__STATIC_INLINE void uart_erbfi_setf(T_HWP_UART_T *hw_uart, uint8_t erbfi)
{
    hw_uart->dlh_ier.ier.bit_field.erbfi = erbfi;
}

__STATIC_INLINE uint32_t uart_iir_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->iir_fcr.iir.val;
}

__STATIC_INLINE void uart_iir_unpack(T_HWP_UART_T *hw_uart, uint8_t* fifose, uint8_t* iid)
{
    T_UART_IIR localVal;

    localVal = hw_uart->iir_fcr.iir;
    *fifose = localVal.bit_field.fifose;
    *iid = localVal.bit_field.iid;
}

__STATIC_INLINE uint8_t uart_fifose_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->iir_fcr.iir.bit_field.fifose;
}

__STATIC_INLINE uint8_t uart_iid_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->iir_fcr.iir.bit_field.iid;
}

__STATIC_INLINE uint32_t uart_fcr_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->iir_fcr.fcr.val;
}

__STATIC_INLINE void uart_fcr_set(T_HWP_UART_T *hw_uart, uint32_t value)
{
    hw_uart->iir_fcr.fcr.val = value;
}

__STATIC_INLINE void uart_fcr_fifo_enable(T_HWP_UART_T *hw_uart, uint8_t enable)
{
    hw_uart->iir_fcr.fcr.bit_field.fifoe = enable;
}

__STATIC_INLINE void uart_fcr_rfifo_reset(T_HWP_UART_T *hw_uart)
{
    hw_uart->iir_fcr.fcr.bit_field.rfifor = 1;
}

__STATIC_INLINE void uart_fcr_xfifo_reset(T_HWP_UART_T *hw_uart)
{
    hw_uart->iir_fcr.fcr.bit_field.xfifor = 1;
}

__STATIC_INLINE void uart_fcr_dma_enable(T_HWP_UART_T *hw_uart, uint8_t enable)
{
    hw_uart->iir_fcr.fcr.bit_field.dmam = enable;
}

__STATIC_INLINE void uart_fcr_tx_empty_triglvl(T_HWP_UART_T *hw_uart, uint8_t lvl)
{
    hw_uart->iir_fcr.fcr.bit_field.tet = lvl;
}

__STATIC_INLINE void uart_fcr_rcvr_triglvl(T_HWP_UART_T *hw_uart, uint8_t lvl)
{
    hw_uart->iir_fcr.fcr.bit_field.rcvr = lvl;
}

__STATIC_INLINE void uart_fcr_pack(T_HWP_UART_T *hw_uart, uint8_t rcvr, uint8_t tet, uint8_t dmam, uint8_t xfifor, uint8_t rfifor, uint8_t fifoe)
{
    hw_uart->iir_fcr.fcr.val = (((uint32_t)rcvr << 6) | ((uint32_t)tet << 4) | ((uint32_t)dmam << 3) | ((uint32_t)xfifor << 2) | ((uint32_t)rfifor << 1) | ((uint32_t)fifoe << 0));
}

__STATIC_INLINE uint32_t uart_lcr_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lcr.val;
}

__STATIC_INLINE void uart_lcr_set(T_HWP_UART_T *hw_uart, uint32_t value)
{
    hw_uart->lcr.val = value;
}

__STATIC_INLINE void uart_lcr_pack(T_HWP_UART_T *hw_uart, uint8_t dlab, uint8_t break_, uint8_t stick_parity, uint8_t eps, uint8_t pen, uint8_t stop, uint8_t dls)
{
    hw_uart->lcr.val = (((uint32_t)dlab << 7) | ((uint32_t)break_ << 6) | ((uint32_t)stick_parity << 5) | ((uint32_t)eps << 4) | ((uint32_t)pen << 3) | ((uint32_t)stop << 2) | ((uint32_t)dls << 0));
}

__STATIC_INLINE void uart_lcr_unpack(T_HWP_UART_T *hw_uart, uint8_t* dlab, uint8_t* break_, uint8_t* stick_parity, uint8_t* eps, uint8_t* pen, uint8_t* stop, uint8_t* dls)
{
    T_UART_LCR localVal;

    localVal = hw_uart->lcr;
...
__STATIC_INLINE uint8_t uart_dlab_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lcr.bit_field.dlab;
}

__STATIC_INLINE void uart_dlab_setf(T_HWP_UART_T *hw_uart, uint8_t dlab)
{
    hw_uart->lcr.bit_field.dlab = dlab;
}

__STATIC_INLINE uint8_t uart_break_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lcr.bit_field.break_;
}

__STATIC_INLINE void uart_break_setf(T_HWP_UART_T *hw_uart, uint8_t break_)
{
    hw_uart->lcr.bit_field.break_ = break_;
}

__STATIC_INLINE uint8_t uart_stick_parity_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lcr.bit_field.stick_parity;
}

__STATIC_INLINE void uart_stick_parity_setf(T_HWP_UART_T *hw_uart, uint8_t stick_parity)
{
    hw_uart->lcr.bit_field.stick_parity = stick_parity;
}

__STATIC_INLINE uint8_t uart_eps_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lcr.bit_field.eps;
}

__STATIC_INLINE void uart_eps_setf(T_HWP_UART_T *hw_uart, uint8_t eps)
{
    hw_uart->lcr.bit_field.eps = eps;
}

__STATIC_INLINE uint8_t uart_pen_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lcr.bit_field.pen;
}

__STATIC_INLINE void uart_pen_setf(T_HWP_UART_T *hw_uart, uint8_t pen)
{
    hw_uart->lcr.bit_field.pen = pen;
}

__STATIC_INLINE uint8_t uart_stop_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lcr.bit_field.stop;
}

__STATIC_INLINE void uart_stop_setf(T_HWP_UART_T *hw_uart, uint8_t stop)
{
    hw_uart->lcr.bit_field.stop = stop;
}

__STATIC_INLINE uint8_t uart_dls_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lcr.bit_field.dls;
}

__STATIC_INLINE void uart_dls_setf(T_HWP_UART_T *hw_uart, uint8_t dls)
{
    hw_uart->lcr.bit_field.dls = dls;
}

__STATIC_INLINE uint32_t uart_mcr_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->mcr.val;
}

__STATIC_INLINE void uart_mcr_set(T_HWP_UART_T *hw_uart, uint32_t value)
{
    hw_uart->mcr.val = value;
}

__STATIC_INLINE void uart_mcr_pack(T_HWP_UART_T *hw_uart, uint8_t sire, uint8_t afce, uint8_t loopback, uint8_t out2, uint8_t out1, uint8_t rts, uint8_t dtr)
{
    hw_uart->mcr.val = (((uint32_t)sire << 6) | ((uint32_t)afce << 5) | ((uint32_t)loopback << 4) | ((uint32_t)out2 << 3) | ((uint32_t)out1 << 2) | ((uint32_t)rts << 1) | ((uint32_t)dtr << 0));
}

__STATIC_INLINE void uart_mcr_unpack(T_HWP_UART_T *hw_uart, uint8_t* sire, uint8_t* afce, uint8_t* loopback, uint8_t* out2, uint8_t* out1, uint8_t* rts, uint8_t* dtr)
{
    T_UART_MCR localVal;

    localVal = hw_uart->mcr;
...
__STATIC_INLINE uint8_t uart_sire_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->mcr.bit_field.sire;
}

__STATIC_INLINE void uart_sire_setf(T_HWP_UART_T *hw_uart, uint8_t sire)
{
    hw_uart->mcr.bit_field.sire = sire;

}

__STATIC_INLINE uint8_t uart_afce_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->mcr.bit_field.afce;
}

__STATIC_INLINE void uart_afce_setf(T_HWP_UART_T *hw_uart, uint8_t afce)
{
    hw_uart->mcr.bit_field.afce = afce;
}

__STATIC_INLINE uint8_t uart_loopback_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->mcr.bit_field.loopback;
}

__STATIC_INLINE void uart_loopback_setf(T_HWP_UART_T *hw_uart, uint8_t loopback)
{
    hw_uart->mcr.bit_field.loopback = loopback;
}

__STATIC_INLINE uint8_t uart_out2_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->mcr.bit_field.out2;
}

__STATIC_INLINE void uart_out2_setf(T_HWP_UART_T *hw_uart, uint8_t out2)
{
    hw_uart->mcr.bit_field.out2 = out2;
}

__STATIC_INLINE uint8_t uart_out1_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->mcr.bit_field.out1;
}

__STATIC_INLINE void uart_out1_setf(T_HWP_UART_T *hw_uart, uint8_t out1)
{
    hw_uart->mcr.bit_field.out1 = out1;
}

__STATIC_INLINE uint8_t uart_rts_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->mcr.bit_field.rts;
}

__STATIC_INLINE void uart_rts_setf(T_HWP_UART_T *hw_uart, uint8_t rts)
{
    hw_uart->mcr.bit_field.rts = rts;
}

__STATIC_INLINE uint8_t uart_dtr_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->mcr.bit_field.dtr;
}

__STATIC_INLINE void uart_dtr_setf(T_HWP_UART_T *hw_uart, uint8_t dtr)
{
    hw_uart->mcr.bit_field.dtr = dtr;
}

__STATIC_INLINE uint32_t uart_lsr_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lsr.val;
}

__STATIC_INLINE void uart_lsr_unpack(T_HWP_UART_T *hw_uart, uint8_t* addr_rcvd, uint8_t* rfe, uint8_t* temt, uint8_t* thre, uint8_t* bi, uint8_t* fe, uint8_t* pe, uint8_t* oe, uint8_t* dr)
{
    T_UART_LSR localVal;

    localVal = hw_uart->lsr;
...
__STATIC_INLINE uint8_t uart_addr_rcvd_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lsr.bit_field.addr_rcvd;
}

__STATIC_INLINE uint8_t uart_rfe_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lsr.bit_field.rfe;
}

__STATIC_INLINE uint8_t uart_temt_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lsr.bit_field.temt;
}

__STATIC_INLINE uint8_t uart_thre_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lsr.bit_field.thre;
}

__STATIC_INLINE uint8_t uart_bi_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lsr.bit_field.bi;
}

__STATIC_INLINE uint8_t uart_fe_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lsr.bit_field.fe;
}

__STATIC_INLINE uint8_t uart_pe_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lsr.bit_field.pe;
}

__STATIC_INLINE uint8_t uart_oe_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lsr.bit_field.oe;
}

__STATIC_INLINE uint8_t uart_dr_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lsr.bit_field.dr;
}

__STATIC_INLINE uint32_t uart_msr_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->msr.val;
}

__STATIC_INLINE void uart_msr_unpack(T_HWP_UART_T *hw_uart, uint8_t* dcd, uint8_t* ri, uint8_t* dsr, uint8_t* cts, uint8_t* ddcd, uint8_t* teri, uint8_t* ddsr, uint8_t* dcts)
{
    T_UART_MSR localVal;

    localVal = hw_uart->msr;
...
__STATIC_INLINE uint8_t uart_dcd_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->msr.bit_field.dcd;
}

__STATIC_INLINE uint8_t uart_ri_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->msr.bit_field.ri;
}

__STATIC_INLINE uint8_t uart_dsr_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->msr.bit_field.dsr;
}

__STATIC_INLINE uint8_t uart_cts_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->msr.bit_field.cts;
}

__STATIC_INLINE uint8_t uart_ddcd_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->msr.bit_field.ddcd;
}

__STATIC_INLINE uint8_t uart_teri_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->msr.bit_field.teri;
}

__STATIC_INLINE uint8_t uart_ddsr_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->msr.bit_field.ddsr;
}

__STATIC_INLINE uint8_t uart_dcts_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->msr.bit_field.dcts;
}

__STATIC_INLINE uint32_t uart_scr_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->scr.val;
}

__STATIC_INLINE void uart_scr_set(T_HWP_UART_T *hw_uart, uint32_t value)
{
    hw_uart->scr.val = value;
}

__STATIC_INLINE void uart_scr_pack(T_HWP_UART_T *hw_uart, uint8_t scratchpad)
{
    hw_uart->scr.val = (((uint32_t)scratchpad << 0));
}

__STATIC_INLINE void uart_scr_unpack(T_HWP_UART_T *hw_uart, uint8_t* scratchpad)
{
    T_UART_SCR localVal;

    localVal = hw_uart->scr;
    *scratchpad = localVal.bit_field.scratchpad;
}

__STATIC_INLINE uint8_t uart_scratchpad_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->scr.bit_field.scratchpad;
}

__STATIC_INLINE void uart_scratchpad_setf(T_HWP_UART_T *hw_uart, uint8_t scratchpad)
{
    hw_uart->scr.bit_field.scratchpad = scratchpad;
}

__STATIC_INLINE uint32_t uart_lpdll_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lpdll.val;
}

__STATIC_INLINE void uart_lpdll_set(T_HWP_UART_T *hw_uart, uint32_t value)
{
    hw_uart->lpdll.val = value;
}

__STATIC_INLINE void uart_lpdll_pack(T_HWP_UART_T *hw_uart, uint8_t lpdll)
{
    hw_uart->lpdll.val = (((uint32_t)lpdll << 0));
}

__STATIC_INLINE void uart_lpdll_unpack(T_HWP_UART_T *hw_uart, uint8_t* lpdll)
{
    T_UART_LPDLL localVal;

    localVal = hw_uart->lpdll;
    *lpdll = localVal.bit_field.lpdll;
}

__STATIC_INLINE uint8_t uart_lpdll_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lpdll.bit_field.lpdll;
}

__STATIC_INLINE void uart_lpdll_setf(T_HWP_UART_T *hw_uart, uint8_t lpdll)
{
    hw_uart->lpdll.bit_field.lpdll = lpdll;
}

__STATIC_INLINE uint32_t uart_lpdlh_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lpdlh.val;
}

__STATIC_INLINE void uart_lpdlh_set(T_HWP_UART_T *hw_uart, uint32_t value)
{
    hw_uart->lpdlh.val = value;
}

__STATIC_INLINE void uart_lpdlh_pack(T_HWP_UART_T *hw_uart, uint8_t lpdlh)
{
    hw_uart->lpdlh.val = (((uint32_t)lpdlh << 0));
}

__STATIC_INLINE void uart_lpdlh_unpack(T_HWP_UART_T *hw_uart, uint8_t* lpdlh)
{
    T_UART_LPDLH localVal;

    localVal = hw_uart->lpdlh;
    *lpdlh = localVal.bit_field.lpdlh;
}

__STATIC_INLINE uint8_t uart_lpdlh_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->lpdlh.bit_field.lpdlh;
}

__STATIC_INLINE void uart_lpdlh_setf(T_HWP_UART_T *hw_uart, uint8_t lpdlh)
{
    hw_uart->lpdlh.bit_field.lpdlh = lpdlh;
}

__STATIC_INLINE uint32_t uart_far_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->far.val;
}

__STATIC_INLINE void uart_far_unpack(T_HWP_UART_T *hw_uart, uint8_t* fifo_access_mode)
{
    T_UART_FAR localVal;

    localVal = hw_uart->far;
    *fifo_access_mode = localVal.bit_field.fifo_access_mode;
}

__STATIC_INLINE uint8_t uart_fifo_access_mode_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->far.bit_field.fifo_access_mode;
}

__STATIC_INLINE uint32_t uart_tfr_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->tfr.val;
}

__STATIC_INLINE void uart_tfr_unpack(T_HWP_UART_T *hw_uart, uint8_t* transmit_fifo_read)
{
    T_UART_TFR localVal;

    localVal = hw_uart->tfr;
    *transmit_fifo_read = localVal.bit_field.transmit_fifo_read;
}

__STATIC_INLINE uint8_t uart_transmit_fifo_read_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->tfr.bit_field.transmit_fifo_read;
}

__STATIC_INLINE uint32_t uart_rfw_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->rfw.val;
}

__STATIC_INLINE void uart_rfw_set(T_HWP_UART_T *hw_uart, uint32_t value)
{
    hw_uart->rfw.val = value;
}

__STATIC_INLINE void uart_rfw_pack(T_HWP_UART_T *hw_uart, uint8_t rffe, uint8_t rfpe, uint8_t rfwd)
{
    hw_uart->rfw.val = (((uint32_t)rffe << 9) | ((uint32_t)rfpe << 8) | ((uint32_t)rfwd << 0));
}

__STATIC_INLINE uint32_t uart_usr_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->usr.val;
}

__STATIC_INLINE void uart_usr_unpack(T_HWP_UART_T *hw_uart, uint8_t* rff, uint8_t* rfne, uint8_t* tfe, uint8_t* tfnf, uint8_t* busy)
{
    T_UART_USR localVal;

    localVal = hw_uart->usr;
...
__STATIC_INLINE uint8_t uart_rff_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->usr.bit_field.rff;
}

__STATIC_INLINE uint8_t uart_rfne_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->usr.bit_field.rfne;
}

__STATIC_INLINE uint8_t uart_tfe_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->usr.bit_field.tfe;
}

__STATIC_INLINE uint8_t uart_tfnf_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->usr.bit_field.tfnf;
}

__STATIC_INLINE uint8_t uart_busy_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->usr.bit_field.busy;
}
__STATIC_INLINE uint8_t uart_rfl_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->rfl.bit_field.rfl;
}

__STATIC_INLINE uint32_t uart_htx_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->htx.val;
}

__STATIC_INLINE void uart_htx_set(T_HWP_UART_T *hw_uart, uint32_t value)
{
    hw_uart->htx.val = value;
}

__STATIC_INLINE void uart_htx_pack(T_HWP_UART_T *hw_uart, uint8_t halt_tx)
{
    hw_uart->htx.val = (((uint32_t)halt_tx << 0));
}

__STATIC_INLINE void uart_htx_unpack(T_HWP_UART_T *hw_uart, uint8_t* halt_tx)
{
    T_UART_HTX localVal;

    localVal = hw_uart->htx;
    *halt_tx = localVal.bit_field.halt_tx;
}

__STATIC_INLINE uint8_t uart_halt_tx_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->htx.bit_field.halt_tx;
}

__STATIC_INLINE void uart_halt_tx_setf(T_HWP_UART_T *hw_uart, uint8_t halt_tx)
{
    hw_uart->htx.bit_field.halt_tx = halt_tx;
}

__STATIC_INLINE uint32_t uart_dlf_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->dlf.val;
}

__STATIC_INLINE void uart_dlf_set(T_HWP_UART_T *hw_uart, uint32_t value)
{
    hw_uart->dlf.val = value;
}

__STATIC_INLINE void uart_dlf_pack(T_HWP_UART_T *hw_uart, uint8_t dlf)
{
    hw_uart->dlf.val = (((uint32_t)dlf << 0));
}

__STATIC_INLINE void uart_dlf_unpack(T_HWP_UART_T *hw_uart, uint8_t* dlf)
{
    T_UART_DLF localVal;

    localVal = hw_uart->dlf;
    *dlf = localVal.bit_field.dlf;
}

__STATIC_INLINE uint8_t uart_dlf_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->dlf.bit_field.dlf;
}

__STATIC_INLINE void uart_dlf_setf(T_HWP_UART_T *hw_uart, uint8_t dlf)
{
    hw_uart->dlf.bit_field.dlf = dlf;
}

__STATIC_INLINE uint32_t uart_cpr_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->cpr.val;
}

__STATIC_INLINE void uart_cpr_unpack(T_HWP_UART_T *hw_uart, uint8_t* fifo_mode, uint8_t* dma_extra, uint8_t* uart_add_encoded_params, uint8_t* shadow, uint8_t* fifo_stat, uint8_t* fifo_access, uint8_t* new_feat, uint8_t* sir_lp_mode, uint8_t* sir_mode, uint8_t* thre_mode, uint8_t* afce_mode, uint8_t* apb_data_width)
{
    T_UART_CPR localVal;

    localVal = hw_uart->cpr;
...
__STATIC_INLINE uint8_t uart_fifo_mode_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->cpr.bit_field.fifo_mode;
}

__STATIC_INLINE uint8_t uart_dma_extra_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->cpr.bit_field.dma_extra;
}

__STATIC_INLINE uint8_t uart_uart_add_encoded_params_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->cpr.bit_field.uart_add_encoded_params;
}

__STATIC_INLINE uint8_t uart_shadow_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->cpr.bit_field.shadow;
}

__STATIC_INLINE uint8_t uart_fifo_stat_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->cpr.bit_field.fifo_stat;
}

__STATIC_INLINE uint8_t uart_fifo_access_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->cpr.bit_field.fifo_access;
}

__STATIC_INLINE uint8_t uart_new_feat_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->cpr.bit_field.new_feat;
}

__STATIC_INLINE uint8_t uart_sir_lp_mode_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->cpr.bit_field.sir_lp_mode;
}

__STATIC_INLINE uint8_t uart_sir_mode_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->cpr.bit_field.sir_mode;
}

__STATIC_INLINE uint8_t uart_thre_mode_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->cpr.bit_field.thre_mode;
}

__STATIC_INLINE uint8_t uart_afce_mode_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->cpr.bit_field.afce_mode;
}

__STATIC_INLINE uint8_t uart_apb_data_width_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->cpr.bit_field.apb_data_width;
}

__STATIC_INLINE uint32_t uart_ucv_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->ucv.val;
}

__STATIC_INLINE void uart_ucv_unpack(T_HWP_UART_T *hw_uart, uint32_t* uart_component_version)
{
    T_UART_UCV localVal;

    localVal = hw_uart->ucv;
    *uart_component_version = localVal.bit_field.uart_component_version;
}

__STATIC_INLINE uint32_t uart_uart_component_version_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->ucv.bit_field.uart_component_version;
}

__STATIC_INLINE uint32_t uart_ctr_get(T_HWP_UART_T *hw_uart)
{
    return hw_uart->ctr.val;
}

__STATIC_INLINE void uart_ctr_unpack(T_HWP_UART_T *hw_uart, uint32_t* peripheral_id)
{
    T_UART_CTR localVal;

    localVal = hw_uart->ctr;
    *peripheral_id = localVal.bit_field.peripheral_id;
}

__STATIC_INLINE uint32_t uart_peripheral_id_getf(T_HWP_UART_T *hw_uart)
{
    return hw_uart->ctr.bit_field.peripheral_id;
}
#endif
