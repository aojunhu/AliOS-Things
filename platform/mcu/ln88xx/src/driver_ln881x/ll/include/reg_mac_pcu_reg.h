...
typedef union
{
    ...
    struct
    {
        uint32_t                        pa_mode :  1; /* 0: 0, 0 每 PA and PHY are powered down.
1 每 PA and PHY are powered up.*/
        uint32_t                       bss_ibss :  1; /* 1: 1, 1 每 This station is part of Independent BSS.
0 每 This station is part of Infrastructure BSS*/
        uint32_t                         ap_sta :  1; /* 2: 2, 0 每 In Infrastructure BSS this is an access point.
1 每 In Infrastructure BSS this is a station*/
        uint32_t                        edca_en :  1; /* 3: 3, 0 每 EDCA feature disabled. DCF procedure defined in 802.11 is followed.
1 每 EDCA feature is enabled.*/
        uint32_t                        hcca_en :  1; /* 4: 4, 0 每 HCCA feature disabled.
1 每 HCCA feature enabled.*/
        uint32_t                         pcf_en :  1; /* 5: 5, 0 每 PCF feature is disabled.
1 每 PCF feature is enabled.*/
        uint32_t                           qack :  1; /* 6: 6, 0 每 CF-ACK bit set in received non-directed Data frame will not be interpreted.
1 每 CF-ACK bit set in received non-directed Data frame will be
interpreted.CF-Ack bit set in received directed frame will be interpreted regardless of this setting.*/
        uint32_t                   rx_fifo_ctrl :  1; /* 7: 7, 0 每 The FULL indication given by Bus FIFO on RX path is used by PA Data path to transfer the bytes from PHY FIFO to Bus FIFO.
1 每 The FULL indication is disregarded. Bus FIFO may be
overwritten if not read out at the required rate.*/
        uint32_t            tx_head_update_ctrl :  1; /* 8: 8, 0 每 TX head pointer register (pointer to first frame in queue) with the next pointer read from current TX descriptor at completion of transmission.
1 每 TX head pointer register is always updated with NULL at
completion of transmission.*/
        uint32_t            rx_link_update_ctrl :  1; /* 9: 9, 0 每 RX link pointer register (pointer to next RX link element in
queue) with the next pointer read from current RX link element at completion of reception.                                                          1 每 RX link pointer register is always updated with NULL at
completion of reception.*/
        uint32_t        msdu_life_time_check_en :  1; /*10:10, 1 每 Lifetime of the MSDU is validated
0 每 Lifetime of the MSDU is not validated*/
        uint32_t                          ce_en :  1; /*11:11, 1 每 Cipher engine is enabled for encryption/decryption
0 每 Cipher engine is disabled.*/
        uint32_t                  pa_tx_suspend :  1; /*12:12, 1 每 Suspend request. This value triggers the suspension of
...
0 每 Protocol accelerator is not suspended.*/
        uint32_t          disable_duplicate_det :  1; /*13:13, 1 每 Disables duplicate detection in hardware.
0 每 Default. Duplicate detection in hardware is enabled.*/
        uint32_t     phy_register_address_width :  1; /*14:14, 0 每 Lower 7 bits of PHY Register Address only are valid.
1 每 All 8 bits of PHY Register Address are valid and are shifted out of SPI.*/
        uint32_t            fcs_failure_eifs_en :  1; /*15:15, 0 每 Disable performing of EIFS when FCS fails for a frame which was received when PHY CCA was idle.
1 - Enable performing of EIFS when FCS fails for a frame which was received when PHY CCA was idle.*/
        uint32_t               rx_error_eifs_en :  1; /*16:16, 0 每 Disable performing of EIFS when PHY indicates
RX_END.error and CCA was idle.
1 每 Enable performing of EIFS when PHY indicates RX_END.error and CCA was idle.*/
        uint32_t    cca_based_protocol_timer_en :  1; /*17:17, 0 每 Disable CCA based protocol timers. When disabled the slot timers are disabled when reception happens and CCA is idle.
1- Enable CCA based protocol timers. When enabled then the slot timers are enabled even when reception happens without CCA assertion.*/
        uint32_t                 disable_ack_tx :  1; /*18:18, 0 每 Normal mode. ACK is transmitted on reception of FCS valid directed unicast packets.
1 每 ACK transmission is disabled.*/
        uint32_t                 disable_cts_tx :  1; /*19:19, 0 每 Normal mode. CTS is transmitted on reception of FCS valid directed RTS frame when NAV is idle.
1 每 CTS transmission is disabled.*/
        uint32_t      pri_channel_cca_bypass_en :  1; /*20:20, 0 每 MAC uses the CCA indication provided for the Primary
channel by PHY for channel access
1 每 MAC by-passes the CCA indication for the Primary channel by PHY and uses the value provided by software*/
        uint32_t   pri_channel_cca_bypass_value :  1; /*21:21, CCA value to be used for By-passing the CCA indication for
Primary channel
0 每 CCA idle
1 每 CCA busy*/
        uint32_t      sec_channel_cca_bypass_en :  1; /*22:22, 0 每 MAC uses the CCA indication provided for the Secondary
channel by PHY for channel access
1 每 MAC by-passes the CCA indication for the Secondary channel by PHY and uses the value provided by software*/
        uint32_t   sec_channel_cca_bypass_value :  1; /*23:23, CCA value to be used for By-passing the CCA indication for
Secondary channel
0 每 CCA idle
1 每 CCA busy*/
        uint32_t                      ht_sta_en :  1; /*24:24, 1 每 HT STA enable
0 每 Non-HT STA*/
        uint32_t                   mac_clock_en :  1; /*25:25, 1 每 Clock is enabled for MAC H/W blocks
0 每 Clock is disabled (gated) for MAC H/W blocks*/
        uint32_t             defragmentation_en :  1; /*26:26, 1 每 Defragmentation will be done by H/W
0 每 Defragmentation will not be done in H/W*/
        uint32_t              de_aggregation_en :  1; /*27:27, 1 每 De-aggregation will be done by H/W
0 每 De-aggregation will not be done by H/W*/
        uint32_t                       tx_abort :  1; /*28:28, 1 每 Transmission will be aborted and PA will enter aborted state in which no transmission will happen. Any transmission which is in progress will stop abruptly.
0 每 PA will work normally based on the channel access function that is active.*/
        uint32_t           self_cts_on_tx_abort :  1; /*29:29, 1 每 PA will transmit self-CTS with in PIFS after a TX abort
0 每 PA will not transmit self-CTS after a TX abort*/
        uint32_t                  tx_abort_mode :  1; /*30:30, 1 每 TX abort can be initiated only by H/W
0 每 TX abort can be initiated only by S/W*/
...
typedef union
{
    ...
    struct
    {
        uint32_t                            fcs :  1; /* 0: 0, 0 每 FCS check failed.
1 每 FCS check passed.*/
        uint32_t                    cca_primary :  1; /* 1: 1,           0 每 Idle.
1 每 Busy*/
        uint32_t                  cca_secondary :  1; /* 2: 2,          0 每 Idle.
1 每 Busy.*/
        uint32_t             duplicate_detected :  1; /* 3: 3, 0 每 Received frame was not a duplicate.
1 每 Received frame was detected as duplicate.*/
        uint32_t                    beacon_txed :  1; /* 4: 4, 0 每 This means that beacon was not transmitted in this beacon
interval.
1 每 This means that beacon was transmitted in this beacon interval.*/
        uint32_t                    slot_in_use :  1; /* 5: 5, Slot currently in use in the STA.
0 每 Short slot is being used
1 每 Long slot is being used*/
        uint32_t             pa_in_suspend_mode :  1; /* 6: 6, Suspend status of Protocol accelerator
1 每 Protocol accelerator is in suspension mode. It will not initiate any frame exchange. However SIFS response and Beacon transmission are not stopped.
0 每 Protocol accelerator is active and can initiate frame exchanges.*/
        uint32_t      hi_priority_q_access_lock :  1; /* 7: 7, 0 每 The registers storing the pointer to the first frame in Hi Priority Q can be accessed.
1 每 The registers storing the pointer to the first frame in Hi Priority Q should not be accessed*/
        uint32_t            ac_bk_q_access_lock :  1; /* 8: 8, 0 每 The registers storing the pointer to the first frame in AC_BK Q can be accessed.
1 每 The registers storing the pointer to the first frame in AC_BK Q should not be accessed*/
        uint32_t            ac_be_q_access_lock :  1; /* 9: 9, 0 每 The registers storing the pointer to the first frame in AC_BE Q can be accessed.
1 每 The registers storing the pointer to the first frame in AC_BE Q should not be accessed*/
        uint32_t            ac_vi_q_access_lock :  1; /*10:10, 0 每 The registers storing the pointer to the first frame in AC_VI Q can be accessed.
1 每 The registers storing the pointer to the first frame in AC_VI Q should not be accessed*/
        uint32_t            ac_vo_q_access_lock :  1; /*11:11, 0 每 The registers storing the pointer to the first frame in AC_VO Q can be accessed.
1 每 The registers storing the pointer to the first frame in AC_VO Q should not be accessed*/
        uint32_t hcca_sta_priority_0_q_access_lock :  1; /*12:12, 0 每 The registers storing the pointer to the first frame in HCCA STA Priority 0 Q can be accessed.
1 每 The registers storing the pointer to the first frame in HCCA STA Priority 0 Q should not be accessed*/
        uint32_t hcca_sta_priority_1_q_access_lock :  1; /*13:13, 0 每 The registers storing the pointer to the first frame in HCCA STA Priority 1 Q can be accessed.
1 每 The registers storing the pointer to the first frame in HCCA STA Priority 1 Q should not be accessed*/
        uint32_t hcca_sta_priority_2_q_access_lock :  1; /*14:14, 0 每 The registers storing the pointer to the first frame in HCCA STA Priority 2 Q can be accessed.
1 每 The registers storing the pointer to the first frame in HCCA STA Priority 2 Q should not be accessed*/
        uint32_t hcca_sta_priority_3_q_access_lock :  1; /*15:15, 0 每 The registers storing the pointer to the first frame in HCCA STA Priority 3 Q can be accessed.
1 每 The registers storing the pointer to the first frame in HCCA STA Priority 3 Q should not be accessed*/
        uint32_t hcca_sta_priority_4_q_access_lock :  1; /*16:16, 0 每 The registers storing the pointer to the first frame in HCCA STA Priority 4 Q can be accessed.
1 每 The registers storing the pointer to the first frame in HCCA STA Priority 4 Q should not be accessed*/
        uint32_t hcca_sta_priority_5_q_access_lock :  1; /*17:17, 0 每 The registers storing the pointer to the first frame in HCCA STA Priority 5 Q can be accessed.
1 每 The registers storing the pointer to the first frame in HCCA STA Priority 5 Q should not be accessed*/
        uint32_t hcca_sta_priority_6_q_access_lock :  1; /*18:18, 0 每 The registers storing the pointer to the first frame in HCCA STA Priority 6 Q can be accessed.
1 每 The registers storing the pointer to the first frame in HCCA STA Priority 6 Q should not be accessed*/
        uint32_t hcca_sta_priority_7_q_access_lock :  1; /*19:19, 0 每 The registers storing the pointer to the first frame in HCCA STA Priority 7 Q can be accessed.
1 每 The registers storing the pointer to the first frame in HCCA STA Priority 7 Q should not be accessed*/
        uint32_t          hcca_ap_q_access_lock :  1; /*20:20, 0 每 The Software can access the Q pointer LUT from shared
memory.
1 每 The Software should not access the Q pointer LUT from shared memory.*/
        uint32_t              pcf_q_access_lock :  1; /*21:21, 0 每 The Software can access the Q pointer LUT from shared
memory.
1 每 The Software should not access the Q pointer LUT from shared memory.*/
        uint32_t beacon_txed_in_pre_beacon_interval :  1; /*22:22, 0 每 Indicates that Beacon was not transmitted in the previous
Beacon interval.
1 每 Indicates that Beacon was transmitted in the previous Beacon interval.*/
        uint32_t               pa_tx_abort_mode :  1; /*23:23, 0 每 Indicates that PA is not in TX abort mode
1 每 indicates that PA is in TX abort mode*/
        uint32_t   self_cts_tx_done_on_tx_abort :  1; /*24:24, 0 每 Self-CTS transmission is not done after TX abort
1 每 Self-CTS transmission is done after TX abort*/
...
typedef union
{
    ...
    struct
    {
        uint32_t                          value :  5; /* 4: 0, Range is 1 每 16. This is the number of words transferred in a
...
b4:b3 00: HT_BW20 每 20 MHz
01: HT_BW40 每 40 MHz
10: HT_BW_20DL 每 Duplicate Legacy
11: HT_BW_20DH 每 Duplicate HT
...
typedef union
{
    ...
    struct
    {
        uint32_t                 corrupt_tx_fcs :  1; /* 0: 0, 1 每 FCS of transmitted frame will be corrupted by H/W
0 每 FCS of transmitted frame will not be corrupted by H/W*/
        uint32_t                        skip_sn :  1; /* 1: 1, 1 每 Sequence number will not be generated for TX frames.
0 每 Sequence number will be generated for TX frames.*/
...
typedef union
{
    ...
    struct
    {
        uint32_t                     touch_mpdu :  1; /* 0: 0, 0 每 H/W will insert relevant field in the MPDUs during transmission
1 每 H/W will not insert any field in the MPDU during transmission*/
        uint32_t            rx_buffer_based_ack :  1; /* 1: 1, 0 每 Rx Buffer Based ACK is not enabled.
1 每 Rx Buffer Based ACK is enabled*/
        uint32_t                sync_bus_pa_clk :  1; /* 2: 2, 0 每 Pa Clk and the AHB Master clocks are not synchronous
1 每 Pa Clk and the AHB Master clocks are synchronous*/
        uint32_t  sifs_responses_at_non_ht_rate :  1; /* 3: 3, 0 每 SIFS responses generated by H/W will be at non-HT Reference rate
1 每 SIFS responses generated by H/W will be at non-HT Basic rate*/
...
typedef union
{
    ...
    struct
    {
        uint32_t                            ack :  1; /* 0: 0, 1 每 Enable filtering ACK frames.
0 每 Disable filtering ACK frames.*/
        uint32_t                            cts :  1; /* 1: 1, 1 每 Enable filtering CTS frames.
0 每 Disable filtering CTS frames.*/
        uint32_t                            rts :  1; /* 2: 2, 1 每 Enable filtering RTS frames.
0 每 Disable filtering RTS frames.*/
        uint32_t                         beacon :  1; /* 3: 3, 1 每 Enable filtering beacon frames.
0 每 Disable filtering beacon frames.*/
        uint32_t                           atim :  1; /* 4: 4, 1 每 Enable filtering ATIM frames.
0 每 Disable filtering ATIM frames.*/
        uint32_t                         cf_end :  1; /* 5: 5, 1 每 Enable filtering CF_END frames.
0 每 Disable filtering CF_END frames.*/
        uint32_t                       qcf_poll :  1; /* 6: 6, 1 每 Enable filtering QCF_POLL frames.
0 每 Disable filtering QCF_POLL frames.*/
        uint32_t              non_directed_data :  1; /* 7: 7, 1 每 Filter DATA frames not directed to this station.
0 每 Report DATA frames not directed to this station subject to
other filters.*/
        uint32_t           bc_mc_data_other_bss :  1; /* 8: 8, 1 每 Filter BC/MC DATA frames belonging to other BSS.
0 每 Report BC/MC DATA frames from other BSS.*/
        uint32_t               duplicate_frames :  1; /* 9: 9, 1 每 Enable filtering of duplicate frames
0 每 Disable filtering of duplicate frames*/
        uint32_t                     fcs_failed :  1; /*10:10, 1 每 Enable filtering of frames whose FCS has failed
0 每 Disable filtering of frames whose FCS has failed*/
        uint32_t              de_authentication :  1; /*11:11, 1 每 Enable filtering of De-authentication frame
0 每 Disable filtering of De-authentication frame*/
        uint32_t          discarded_ibss_beacon :  1; /*12:12, 1 每 Filter Beacon frames (in IBSS mode) which are not used for adoption because the timestamp field is lower than TSF timer.
0 每 Report all Beacon frames subject to other filters*/
        uint32_t              non_directed_mgmt :  1; /*13:13, 1 每 Filter Management frames which are not directed to current STA
0 每 Non-directed Management frames are reported subject to
other filters*/
        uint32_t              non_directed_ctrl :  1; /*14:14, 1 每 Filter Control frames which are not directed to current STA
0 每 Non-directed Control frames are reported subject to other
filters.*/
        uint32_t             sec_channel_frames :  1; /*15:15, 1 每 Filter frames which are received in secondary channel (20
MHz PPDU from Secondary channel)
0 每 Frames received in secondary channel are reported
irrespective of other filters*/
        uint32_t           bc_mc_mgmt_other_bss :  1; /*16:16, 1 每 Filter BC/MC MGMT frames belonging to other BSS.
0 每 Report BC/MC MGMT frames from other BSS.*/
        uint32_t     non_sifs_response_ba_frame :  1; /*17:17, 1 每 Filter BA frames which are not received as SIFS response
0 每 Report BA frames which are not received as SIFS response*/
        uint32_t          sifs_respone_ba_frame :  1; /*18:18, 1 每 Filter BA frames which are received as SIFS response
0 每 Report BA frames which are received as SIFS response*/
...
typedef union
{
    ...
    struct
    {
        uint32_t               hi_priority_q_en :  1; /* 0: 0, 1 每 Enable High Priority Queue
0 每 Disable High Priority Queue*/
        uint32_t                         beacon :  1; /* 1: 1, 1 每 keep Beacon frame in hi-priority RX Queue
0 每 keep it in normal Queue*/
        uint32_t                  probe_request :  1; /* 2: 2, 1 每 keep Probe Request frame in hi-priority RX Queue
0 每 keep it in normal Queue*/
        uint32_t                 probe_response :  1; /* 3: 3, 1 每 keep Probe Response frame in hi-priority RX Queue
0 每 keep it in normal Queue*/
        uint32_t                           atim :  1; /* 4: 4, 1 每 keep ATIM frame in hi-priority RX Queue
0 每 keep it in normal Queue*/
        uint32_t                     reserved_0 : 27; /*31: 5,                             NA*/
    } bit_field;
...
typedef union
{
    ...
    struct
    {
        uint32_t                         tsf_en :  1; /* 0: 0, 0 每 This function is disabled. TBTT interrupt is not generated.
1 每 This function is enabled. In IBSS STA case, beacon
...
0 每 After being enabled, the TSF waits to receive Beacon or
Probe Response frame with matching BSSID, adopts the
parameters and starts the TSF timer and TBTT generation.
1 每 After being enabled, the TSF immediately starts the TSF
timer and TBTT generation.*/
        uint32_t              beacon_tx_suspend :  1; /* 2: 2, 0 每 Beacon transmission will be performed after TBTT (if
required for the current STA)                                                               1 每 Beacon transmission will be suspended*/
...
typedef union
{
    ...
    struct
    {
        uint32_t                         length : 16; /*15: 0,            Range is 1 每 65535*/
...
b4:b3  00: HT_BW20 每 20 MHz 01: HT_BW40 每 40 MHz 10: HT_BW_20DL 每 Duplicate Legacy 11: HT_BW_20DH 每 Duplicate HT
...
typedef union
{
    ...
    struct
    {
        uint32_t                           mode :  2; /* 1: 0, 00 每 802.11b physical layer 01 每 802.11a physical layer 10 每 802.11g Compatibility 11 每 802.11g High Rate*/
        uint32_t              enhanced_nav_prot :  1; /* 2: 2, 0 每 No enhanced protection 1 每 This overrides RTS threshold and mode bits. Protection frame is transmitted for every burst (fragmentation burst or TXOP burst) This is valid only for 11e (any Mode)*/
        uint32_t                  nav_port_type :  1; /* 3: 3, 0 每 RTS/CTS 1 每 Self CTS This is valid only if Enhanced NAV Protection is set.*/
        uint32_t                       erp_prot :  1; /* 4: 4, 0 每 No ERP protection 1 每 Protection required for OFDM frames if Mode = 10 This is valid only for Mode = 10*/
        uint32_t                  erp_prot_type :  1; /* 5: 5, 0 每 RTS/CTS 1 每 Self CTS This is valid only if ERP Protection is set.*/
        uint32_t                     reserved_0 : 26; /*31: 6,                             NA*/
    } bit_field;
...
typedef union
{
    ...
    struct
    {
        uint32_t                          value : 16; /*15: 0, Minimum value 每 0. This will enable RTS transmission for all lengths. Maximum value 每 3000. This will disable RTS transmission for all lengths.*/
        uint32_t                     reserved_0 : 16; /*31:16,                             NA*/
    } bit_field;
...
typedef union
{
    ...
    struct
    {
        uint32_t                          value : 32; /*31: 0, b2:b0  000: 11b Short Preamble 001: 11b Long Preamble 010: 11a 011: Reserved 100: Reserved 101: Reserved 110: HT Mixed Mode Frame 111: HT Green Field Frameb4:b3  00: HT_BW20 每 20 MHz 01: HT_BW40 每 40 MHz 10: HT_BW_20DL 每 Duplicate Legacy 11: HT_BW_20DH 每 Duplicate HT
b6:b5  00: 40 MHz Band 01: Upper 20 MHz in 40 MHz Band 10: Reserved 11: Lower 20 MHz in 40 MHz Band
b7  0: Smoothing is not recommended 1: Smoothing is recommended
b8  0: A sounding packet 1: Not a sounding packet
b9  0: A packet without MPDU aggregation 1: A packet with MPDU aggregation
...
typedef union
{
    ...
    struct
    {
        ...
        uint32_t               long_nav_support :  1; /* 2: 2, 0 角 Long NAV supported 1 角 Long NAV not supported*/
        uint32_t                   ht_prot_type :  2; /* 4: 3, 0 角 RTS-CTS at non-HT rate 1 角 First frame exchange at non-HT rate 2 角 LSIG TXOP 3 角 Mixed format at non-HT rate*/
        uint32_t             non_gf_sta_present :  1; /* 5: 5, 0 角 Non GF HT STA present 1 角 Non GF HT STA not present*/
        uint32_t                  rifs_burst_en :  1; /* 6: 6, 0 角 MAC HW will not perform RIFS burst 1 角 MAC HW will perform RIFS burst*/
        uint32_t                disable_rts_cts :  1; /* 7: 7, 0 角MAC H/W will perform RTS-CTS frame exchange if the first frame in the TXOP is of Block ACK or NO_ACK type 1 角MAC H/W will not perform RTS-CTS frame exchange if the first frame in the TXOP is of Block ACK or NO_ACK type*/
        uint32_t                      fc_sta_en :  1; /* 8: 8, 0 角 20/40 MHz Co-existence mode of operation is disabled 1 角 20/40 MHz Co-existence mode of operation is enabled.*/
        uint32_t           txop_promotion_40mhz :  1; /* 9: 9, This bit is applicable only when FC STA is enabled 0 角TXOP promotions are not enabled. MAC H/W will not try to promote a 20 MHz TXOP into a 40 MHz TXOP 1 角 MAC H/W tries to promote every TXOP into a 40 MHz TXOP. If secondary channel is idle for programmed number of slots when backoff is complete in primary channel then MAC H/W starts a 40 MHz TXOP irrespective of the channel bandwidth of the first frame in the TX queue. In order to do this, MAC H/W uses non-HT duplicate (either the given frame or RTS-CTS) frames if the first frame is not already a 40 MHz PPDU.*/
        uint32_t ppdu_demotion_in_start_txop_40mhz :  1; /*10:10, This bit is applicable only when FC STA is enabled 0 每 TXOP demotions are not enabled. MAC H/W will relinquish a TXOP is the first frame in the TX queue is a 40 MHz frame and secondary channel is busy when backoff is complete in primary channel 1 每 MAC H/W will try to start a 20 MHz TXOP when secondary channel is busy and the first frame in the TXOP is a 40 MHz frame. It does this by changing the channel bandwidth of the frame from 40 MHz to 20 MHz. (In order to establish the type of TXOP, H/W senses the Secondary channel CCA one slot prior to the actual backoff slot. It decides the CH_BW of the first frame in the TXOP based on this. It validates the decision during the actual backoff slot and starts a TXOP accordingly).*/
        uint32_t ppdu_demotion_in_mid_txop_40mhz :  1; /*11:11, This bit is applicable only when FC STA is enabled 0 每 PPDU demotions are not performed. When a 20 MHz TXOP is in progress and the next frame in the TX queue is a 40 MHz frame then the TXOP is relinquished. 1 每 When a 20 MHz TXOP is in progress then MAC H/W demotes all 40 MHz PPDU to 20 MHz PPDU and continues the TXOP burst without relinquishing.*/
        uint32_t                   fc_prot_type :  1; /*12:12, The protection type specified below will be used when a protection frame exchange needs to be performed to promote the TXOP to a 40MHz TXOP. The FC protection mechanism will be used when the first frame in the TX Queue is a 20 MHz HT frame. 0 每 RTS/CTS 1 每 Self CTS*/
        uint32_t             sec_channel_offset :  1; /*13:13, 0 每 Secondary channel corresponds to Upper 20 MHz in 40 MHz Band 1 每 Secondary channel corresponds to Lower 20 MHz in 40 MHz Band (This field is not used by H/W when FC is disabled)*/
        uint32_t   first_frame_txop_limit_check :  1; /*14:14, 0 每 TXOP limit check is not done for the first frame in the TXOP 1- First frame in the TXOP is subjected to TXOP limit*/
        uint32_t             lsig_nav_update_en :  1; /*15:15, 0 每 MAC HW does not update NAV based on information from LSIG field from HT_MF frames 1 每 MAC HW updates NAV based on information from LSIG field from HT_MF frames*/
        uint32_t                     reserved_0 : 16; /*31:16,                             NA*/
    } bit_field;
...
typedef union
{
    ...
    struct
    {
        ...
        uint32_t                     read_write :  1; /* 1: 1, 0 每 LUT write operation 1 每 LUT read operation*/
        uint32_t                      lut_index :  8; /* 9: 2, Index to be used for the LUT operation*/
        uint32_t        mini_mpdu_start_spacing :  3; /*12:10, Minimum MPDU start spacing supported by the receiver Set to 0 for no restriction Set to 1 for 1/4 米s Set to 2 for 1/2 米s Set to 3 for 1 米s Set to 4 for 2 米s Set to 5 for 4 米s Set to 6 for 8 米s Set to 7 for 16 米s*/
        uint32_t            max_rx_amdpu_factor :  2; /*14:13, The Maximum RX A-MPDU defined by this field is equal to 2(13 + Maximum Rx A-MPDU Factor) 每 1 octets. Maximum Rx A-MPDU Factor is an integer in the range 0 to 3.*/
        uint32_t                   max_num_aggr :  8; /*22:15, Maximum number of aggregations H/W will perform in an AMPDU*/
        uint32_t                   mpdu_aggr_en :  1; /*23:23, 0 每 MPDU aggregation will not be performed 1 每 MPDU aggregation will be performed*/
...
typedef union
{
    ...
    struct
    {
        uint32_t             table_operation_en :  1; /* 0: 0, 1 每 A zero to one transition of this bit will start the table update procedure. After table is updated this bit is reset to zero. 0 每 No action. Table updates procedure over.*/
        uint32_t                     read_write :  1; /* 1: 1, 0 每 Read from table. 1 每 Write to table.*/
        uint32_t                     reserved_3 :  1; /* 2: 2,                             NA*/
        uint32_t                     reserved_2 :  1; /* 3: 3,                             NA*/
        uint32_t                            tid :  4; /* 7: 4, Range is 0 每 15. This is used while reading from table. The entry corresponding to the TID is read out.*/
        uint32_t                          index :  5; /*12: 8, Range is 0 每 127. 128 STA entries are possible in the table*/
...
typedef union
{
    ...
    struct
    {
        uint32_t                       pcf_mode :  2; /* 1: 0, 00 每 Non CF-Pollable STA. 01 每 CF-Pollable STA. 10 每 Point Coordinator is resident at this STA. 11 每 Reserved.*/
        uint32_t                   piggyback_en :  1; /* 2: 2, 0 每 Subtypes containing ※+CF-Poll§ or ※+CF-Ack§ will not be used. 1 每 Above subtypes are used.*/
        uint32_t                     reserved_0 : 29; /*31: 3,                             NA*/
    } bit_field;
...
typedef union
{
    ...
    struct
    {
        uint32_t                      pm_status :  1; /* 0: 0,       0 每 Active 1 每 PS Mode*/
        uint32_t                       rx_dtims :  1; /* 1: 1, 1 每 Wake up at every Listen Interval only 0 每 Wake up to receive all DTIM beacons*/
...
typedef union
{
    ...
    struct
    {
        ...
        uint32_t                           mode :  2; /* 9: 8, Specifies the SMPS power save mode of the current STA 0 每 Reserved 1 每 Current STA will be put in Static SMPS mode 2 每 Current STA will be put in Dynamic SMPS mode 3 每 SMPS is disabled in current STA*/
        uint32_t                     reserved_0 : 22; /*31:10,                             NA*/
    } bit_field;
...
typedef union
{
    ...
    struct
    {
        uint32_t                    rx_complete :  1; /* 0: 0, 0 每 Interrupt not raised. 1 每 Interrupt is raised when reception of data bits is completed and transferred to Shared Memory with RX descriptor. Reception of certain frame types will not generate interrupt. This depends on RX frame filter register setting. This interrupt is raised also when memory is not available to store the received packet.*/
        uint32_t                    tx_complete :  1; /* 1: 1, 0 每 Interrupt not raised 1 每 Interrupt is raised when successful transmission occurs or when retry limit is reached or if packet lifetime expires. The status is available in the TX Descriptor of the packet.*/
        uint32_t                           tbtt :  1; /* 2: 2, 0 每 Interrupt not raised. 1 每 Interrupt is raised before beacon transmission is expected.*/
        uint32_t            hcca_txop_interrupt :  1; /* 3: 3, 0 每 Interrupt not raised. 1 每 Interrupt is raised at start of TXOP in HC, or when CF-Poll is received in QSTA. In QSTA it is raised also at end of TXOP.*/
        uint32_t                 atim_interrupt :  1; /* 4: 4, 0 每 Interrupt not raised. 1 每 Interrupt is raised at the end of ATIM window.*/
        uint32_t              wake_up_interrupt :  1; /* 5: 5, 0 每 Interrupt is not raised. 1 每 Interrupt is raised at start of S-APSD SP.*/
        uint32_t                          error :  1; /* 6: 6, 0 每 Interrupt is not raised. 1 每 Interrupt is raised when error condition occurs in Hardware. The Error code register has the cause for error. This bit is set when one of the bits is set in Error Interrupt Status register and its corresponding mask bit is not set in the Error Interrupt Mask register. To clear the interrupt, first clear the bit in the Error Interrupt Status register by writing a ?1? to it. Then clear this bit by writing a ?1? to it.*/
        uint32_t              cap_end_interrupt :  1; /* 7: 7, 0 每 Interrupt is not raised. 1 每 Interrupt is raised at the end of a CAP.*/
        uint32_t               cf_end_interrupt :  1; /* 8: 8, 0 每 Interrupt is not raised. 1 每 Interrupt is raised at the end of a CAP.*/
        uint32_t        rf_vco_unlock_interrupt :  1; /* 9: 9, 0 每 Interrupt is not raised. 1 每 Interrupt is raised when RF VCO Unlock indication is obtained through an input from RF.*/
        uint32_t      hi_priority_q_rx_complete :  1; /*10:10, 0 每 Interrupt is not raised. 1 每 Interrupt is raised at the end of reception of a frame in Hi priority RX queue*/
        uint32_t      hw_tx_abort_request_start :  1; /*11:11, 0 每 Interrupt is not raised. 1 每 External H/W trigger for TX abort is asserted*/
        uint32_t        hw_tx_abort_request_end :  1; /*12:12, 0 每 Interrupt is not raised. 1 每 External H/W trigger for TX abort is de-asserted*/
        uint32_t                 radar_detected :  1; /*13:13, 0 每 Interrupt is not raised. 1 每 Radar signal is detected*/
        uint32_t                  pa_tx_suspend :  1; /*14:14, 0 每 Interrupt is not raised. 1 每 PA went into TX suspend mode*/
        uint32_t              de_authentication :  1; /*15:15, 0 每 Interrupt is not raised. 1 每 The current STA is de-authenticated by the AP*/
        uint32_t         start_of_absent_period :  1; /*16:16, 0 每 Interrupt is not raised. 1 每 Absent period has started (P2P)*/
        uint32_t           end_of_absent_period :  1; /*17:17, 0 每 Interrupt is not raised. 1 每 Absent period has ended (P2P)*/
        uint32_t                end_of_ctwindow :  1; /*18:18, 0 每 Interrupt is not raised. 1 每 CTWindow has ended (P2P)*/
        uint32_t                     reserved_0 : 13; /*31:19,                             NA*/
    } bit_field;
...
typedef union
{
    ...
    struct
    {
        uint32_t                    rx_complete :  1; /* 0: 0, 0 每 Interrupt is not masked 1 每 Interrupt is masked*/
        uint32_t                    tx_complete :  1; /* 1: 1, 0 每 Interrupt is not masked 1 每 Interrupt is masked*/
        uint32_t                           tbtt :  1; /* 2: 2, 0 每 Interrupt is not masked 1 每 Interrupt is masked*/
        uint32_t            hcca_txop_interrupt :  1; /* 3: 3, 0 每 Interrupt is not masked 1 每 Interrupt is masked*/
        uint32_t                 atim_interrupt :  1; /* 4: 4, 0 每 Interrupt is not masked 1 每 Interrupt is masked*/
        uint32_t              wake_up_interrupt :  1; /* 5: 5, 0 每 Interrupt is not masked 1 每 Interrupt is masked*/
        uint32_t                          error :  1; /* 6: 6, 0 每 Interrupt is not masked 1 每 Interrupt is masked*/
        uint32_t              cap_end_interrupt :  1; /* 7: 7, 0 每 Interrupt is not masked 1 每 Interrupt is masked*/
        uint32_t               cf_end_interrupt :  1; /* 8: 8, 0 每 Interrupt is not masked 1 每 Interrupt is masked*/
        uint32_t        rf_vco_unlock_interrupt :  1; /* 9: 9, 0 每 Interrupt is not masked 1 每 Interrupt is masked*/
        uint32_t      hi_priority_q_rx_complete :  1; /*10:10, 0 每 Interrupt is not masked 1 每 Interrupt is masked*/
        uint32_t      hw_tx_abort_request_start :  1; /*11:11, 0 每 Interrupt is not masked 1 每 Interrupt is masked*/
        uint32_t        hw_tx_abort_request_end :  1; /*12:12, 0 每 Interrupt is not masked 1 每 Interrupt is masked*/
        uint32_t                 radar_detected :  1; /*13:13, 0 每 Interrupt is not masked 1 每 Interrupt is masked*/
        uint32_t                  pa_tx_suspend :  1; /*14:14, 0 每 Interrupt is not masked 1 每 Interrupt is masked*/
        uint32_t              de_authentication :  1; /*15:15, 0 每 Interrupt is not masked 1 每 Interrupt is masked*/
        uint32_t         start_of_absent_period :  1; /*16:16, 0 每 Interrupt is not masked 1 每 Interrupt is masked*/
        uint32_t           end_of_absent_period :  1; /*17:17, 0 每 Interrupt is not masked 1 每 Interrupt is masked*/
        uint32_t                end_of_ctwindow :  1; /*18:18, 0 每 Interrupt is not masked 1 每 Interrupt is masked*/
        uint32_t                     reserved_0 : 13; /*31:19,                             NA*/
    } bit_field;
...
typedef union
{
    ...
    struct
    {
        uint32_t                          value :  5; /* 4: 0, 0 每 NO_ERROR 
1 每 UNEXPECTED_TX_MAC_HANG 
2 每 UNEXPECTED_TX_Q_EMPTY 
3 每 TXBURSTPROC_ERROR
4 每 BUS_ERROR 
5 每 BUS_WRAP_SIG_ERROR 
10 每 UNEXPECTED_NON_PENDING_MSDU
11 每 UNEXPECTED_MSDU_ADDR 
12 每 UNEXPECTED_RX_Q_EMPTY 
13 每 TX_INTR_FIFO_OVERRUN 
14 每 RX_INTR_FIFO_OVERRUN 
15 每 HIRX_INTR_FIFO_OVERRUN 
16 每 UNEXPECTED_HIRX_Q_EMPTY 
17 每 RX_DATAPATH_HANG_FCS_FAIL_TIMEOUT 
18 每 RX_DATAPATH_HANG_FCS_PASS_TIMEOUT 
19 每 UNEXPECTED_TX_PHY_HANG Others 每 Reserved*/
        uint32_t                     reserved_0 : 27; /*31: 5,                             NA*/
    } bit_field;
...
typedef union
{
    ...
    struct
    {
        ...
        uint32_t                           ec_1 :  1; /* 1: 1, 0 每 Interrupt not raised. 1 每 Interrupt is raised when UNEXPECTED_TX_MAC_HANG internal error occurs.*/
        uint32_t                           ec_2 :  1; /* 2: 2, 0 每 Interrupt not raised 1 每 Interrupt is raised when UNEXPECTED_TX_Q_EMPTY internal error occurs.*/
        uint32_t                           ec_3 :  1; /* 3: 3, 0 每 Interrupt not raised. 1 每 Interrupt is raised when TXBURSTPROC_ERROR internal error occurs.*/
        uint32_t                           ec_4 :  1; /* 4: 4, 0 每 Interrupt not raised. 1 每 Interrupt is raised when BUS_ERROR internal error occurs.*/
        uint32_t                           ec_5 :  1; /* 5: 5, 0 每 Interrupt not raised. 1 每 Interrupt is raised when BUS_WRAP_SIG_ERROR internal error occurs.*/
        ...
        uint32_t                          ec_10 :  1; /*10:10, 0 每 Interrupt is not raised. 1 每 Interrupt is raised when UNEXPECTED_NON_PENDING_MSDU internal error occurs.*/
        uint32_t                          ec_11 :  1; /*11:11, 0 每 Interrupt is not raised. 1 每 Interrupt is raised when UNEXPECTED_MSDU_ADDR internal error occurs.*/
        uint32_t                          ec_12 :  1; /*12:12, 0 每 Interrupt is not raised. 1 每 Interrupt is raised when UNEXPECTED_RX_Q_EMPTY internal error occurs.*/
        uint32_t                          ec_13 :  1; /*13:13, 0 每 Interrupt is not raised. 1 每 Interrupt is raised when TX_INTR_FIFO_OVERRUN internal error occurs.*/
        uint32_t                          ec_14 :  1; /*14:14, 0 每 Interrupt is not raised. 1 每 Interrupt is raised when RX_INTR_FIFO_OVERRUN internal error occurs.*/
        uint32_t                          ec_15 :  1; /*15:15, 0 每 Interrupt is not raised. 1 每 Interrupt is raised when HIRX_INTR_FIFO_OVERRUN internal error occurs.*/
        uint32_t                          ec_16 :  1; /*16:16, 0 每 Interrupt is not raised. 1 每 Interrupt is raised when UNEXPECTED_HIRX_Q_EMPTY internal error occurs.*/
        uint32_t                          ec_17 :  1; /*17:17, 0 每 Interrupt is not raised. 1 每 Interrupt is raised when RX_DATAPATH_HANG_FCS_FAIL_TIMEOUT internal error occurs*/
        uint32_t                          ec_18 :  1; /*18:18, 0 每 Interrupt is not raised. 1 每 Interrupt is raised when RX_DATAPATH_HANG_FCS_PASS_TIMEOUT internal error occurs.*/
        uint32_t                          ec_19 :  1; /*19:19, 0 每 Interrupt not raised. 1 每 Interrupt is raised when UNEXPECTED_TX_PHY_HANG internal error occurs.*/
...
typedef union
{
    ...
    struct
    {
        ...
        uint32_t        phy_register_read_write :  1; /* 8: 8, 0 - Indicates the write operation to be initiated. 1 每 Indicates the read operation to be initiated*/
...
typedef union
{
    ...
    struct
    {
        uint32_t                          value :  1; /* 0: 0, 0 每 Software can access PHY registers through SPI. PA will not have access. 1 每 Software cannot access PHY registers through SPI. PA will have the access.*/
...
typedef union
{
    ...
    struct
    {
        uint32_t                       spi_busy :  1; /* 0: 0, 0 每 SPI is ready for another read/write. 1 每 SPI is busy with current transaction.*/
        uint32_t                     reserved_0 : 31; /*31: 1,                             NA*/
    } bit_field;
...
typedef union
{
    ...
    struct
    {
        uint32_t                    rf_spi_busy :  1; /* 0: 0, 0 每 RF SPI is ready for another read/write. 1 每 RF SPI is busy with current transaction. This is set when RF VCO SPI Control register is written.*/
        uint32_t                     reserved_0 : 31; /*31: 1,                             NA*/
    } bit_field;
...
typedef union
{
    ...
    struct
    {
        ...
        uint32_t                     read_write :  1; /* 1: 1, This flag indicates whether the requested register access is of read/write type. 0 每 Write operation 1 每 Read operation*/
...
typedef union
{
    ...
    struct
    {
        uint32_t              invert_phy_tx_clk :  1; /* 0: 0, 0 每 Do not invert Phy Tx Clk 1 每 Invert Phy Tx Clk*/
        uint32_t           invert_phy_regif_clk :  1; /* 1: 1, 0 每 Do not invert Phy RegIf Clk 1 每 Invert Phy RegIf Clk*/
        uint32_t                     reserved_0 : 30; /*31: 2,                             NA*/
    } bit_field;
...
typedef union
{
    ...
    struct
    {
        uint32_t                          value : 16; /*15: 0, Bits 47 每 32 of the peer station address with which block Ack mechanism is established. When read operation is enabled, HW will read the address programmed in LUT for given index and put bits 47 每 32 of the read address in this REG.*/
        uint32_t                     reserved_0 : 16; /*31:16,                             NA*/
    } bit_field;
...
typedef union
{
    ...
    struct
    {
        uint32_t                          value : 32; /*31: 0, Bits 31 每 0 of the peer station address with which block Ack mechanism is established. When read operation is enabled, HW will read the address programmed in LUT for given index and put bits 31 每 0 of the read address in this REG.*/
    } bit_field;
...
typedef union
{
    ...
    struct
    {
        ...
        uint32_t                  lut_operation :  2; /* 2: 1, 00 角 LUT write operation. 01 角 LUT read operation. 10 角 Entry removal operation. 11 角 Reserved.*/
...
typedef union
{
    ...
    struct
    {
        uint32_t                          ba_en :  1; /* 0: 0, 0 每 Feature is disabled 1 每 Feature is enabled*/
        uint32_t                         lut_en :  1; /* 1: 1, This bit, when set will enable LUT updating operation. HW will clear this bit after updating LUT. 0 每 Idle. 1 每 Will trigger LUT operation.*/
        uint32_t                  lut_operation :  1; /* 2: 2, 1 角 Write entry 0 角 Read entry.*/
        uint32_t                      lut_index :  4; /* 6: 3,     Index of LUT for updating.*/
        uint32_t                     reserved_0 : 25; /*31: 7,                             NA*/
    } bit_field;
...
typedef union
{
    ...
    struct
    {
        uint32_t                          value : 16; /*15: 0, Bits 47 每 32 of the peer station address with which block Ack mechanism is established.*/
        uint32_t                     reserved_0 : 16; /*31:16,                             NA*/
    } bit_field;
...
typedef union
{
    ...
    struct
    {
        uint32_t                          value : 32; /*31: 0, Bits 31 每 0 of the peer station address with which block Ack mechanism is established.*/
    } bit_field;
...
typedef union
{
    ...
    struct
    {
        ...
        uint32_t                  partial_state :  6; /*21:16, Indicates if the state mode is partial or full. 0 角 Full state operation. 1 角 Partial state operation.*/
...
typedef union
{
    ...
    struct
    {
        uint32_t                          value : 32; /*31: 0, Bits 63 每 32 of the bitmap for the BA session.. When read operation is enabled, HW will read the bitmap in LUT for given index and put bits 63 每 32 of the read value in this register.*/
    } bit_field;
...
typedef union
{
    ...
    struct
    {
        uint32_t                          value : 32; /*31: 0, Bits 31 每 0 of the bitmap for the BA session.. When read operation is enabled, HW will read the bitmap in LUT for given index and put bits 31 每 0 of the read value in this register.*/
    } bit_field;
...
typedef union
{
    ...
    struct
    {
        uint32_t                         p2p_en :  1; /* 0: 0, 0 每 P2P is not enabled. 1 每 P2P is enabled.*/
        uint32_t                         ops_en :  1; /* 1: 1, 0 每 Opportunistic power save is not enabled. 1 每 Opportunistic power save is enabled.*/
        uint32_t                           mode :  1; /* 2: 2,   0 每 Group Owner 1 每 Client*/
        uint32_t          ctwindow_txop_allowed :  1; /* 3: 3, 0 每 TXOP is not allowed to cross the end of CTWindow. 1 每 TXOP is allowed to cross the end of CTWindow.*/
        ...
        uint32_t                     cancel_noa :  1; /*28:28, 0 每 No action 1 每 Immediately stops existing NOA schedules and HW updates their status as ※CANCELLED§.*/
...
typedef union
{
    ...
    struct
    {
        ...
        uint32_t                    noa1_status :  2; /*17:16, Status of NOA1 schedule descriptor. SW should set it to ※NEW§ when programming a new descriptor. 00 每 EXPIRED 01 每 CURRENT 10 每 NEW 11 每 CANCELLED*/
        uint32_t                    noa2_status :  2; /*19:18, Status of NOA2 schedule descriptor. SW should set it to ※NEW§ when programming a new descriptor. 00 每 EXPIRED 01 每 CURRENT 10 每 NEW 11 每 CANCELLED*/
        uint32_t                     reserved_0 : 12; /*31:20,                             NA*/
    } bit_field;
...
#define hwp_mac_pcu_reg ((T_HWP_MAC_PCU_REG_T*)REG_MAC_PCU_REG_BASE)
