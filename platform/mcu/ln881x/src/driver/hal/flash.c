...
uint8_t FLASH_DualRead(uint32_t offset, uint32_t length, uint8_t *buffer)
{
    uint8_t shift = ((uint32_t)buffer & 0x03) ? (4 - (uint32_t)buffer & 0x03) : 0;
    uint32_t length_in_words = (length - shift) / sizeof(uint32_t);
    uint8_t remainder = (length - shift) % sizeof(uint32_t);
    ...
    assert_param( (offset + length) <= FALSH_SIZE_MAX);
    assert_param(length_in_words <= QSPI_READ_MAX_LENGTH);
    ...
    if(length_in_words > 0){
        qspi_flash_dual_read_word((uint32_t *)buffer, length_in_words, FLASH_DUAL_FAST_READ, offset, FLASH_QUAD_READ_DUMMY_CLOCKS);
        buffer += sizeof(uint32_t)*length_in_words;
        offset += sizeof(uint32_t)*length_in_words;
    }
    if(remainder){
...
uint8_t FLASH_QuadRead(uint32_t offset, uint32_t length, uint8_t *buffer)
{
    uint8_t shift = ((uint32_t)buffer & 0x03) ? (4 - (uint32_t)buffer & 0x03) : 0;
    uint32_t length_in_words = (length - shift) / sizeof(uint32_t);
    uint8_t remainder = (length - shift) % sizeof(uint32_t);
    ...
    if(length_in_words > 0){
        qspi_flash_quad_read_word((uint32_t *)buffer, length_in_words, FLASH_QUAD_FAST_READ, offset, FLASH_QUAD_READ_DUMMY_CLOCKS);
        buffer += sizeof(uint32_t)*length_in_words;
        offset += sizeof(uint32_t)*length_in_words;
    }
...
void FLASH_LockOTP(void)
{
    ...
    qspi_standard_read_byte( &status.reg1.reg1_data, sizeof(uint8_t), &read_sr1_cmd, sizeof(read_sr1_cmd));
    qspi_standard_read_byte( &status.reg2.reg2_data, sizeof(uint8_t), &read_sr2_cmd, sizeof(read_sr2_cmd));

    status.reg2.LB = 1;

    write_sr_cmd[0] = FLASH_WRITE_STATUS_REG_1;
    write_sr_cmd[1] = status.reg1.reg1_data;
    write_sr_cmd[2] = status.reg2.reg2_data;
