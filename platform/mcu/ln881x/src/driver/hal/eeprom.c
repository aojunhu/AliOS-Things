...
#if (FLASH_OP_QUAL_MODE == ENABLE)
    /* Get Page0 status */
    FLASH_QuadRead(PAGE0_BASE_ADDRESS, sizeof(uint16_t), (uint8_t *)&PageStatus0);
    /* Get Page1 status */
    FLASH_QuadRead(PAGE1_BASE_ADDRESS, sizeof(uint16_t), (uint8_t *)&PageStatus1);
#else
    /* Get Page0 status */
    ...
    switch (PageStatus0) {
        case ERASED:
            if (PageStatus1 == VALID_PAGE) { 
                ...
                value = VALID_PAGE;
#if (FLASH_OP_QUAL_MODE == ENABLE)
                FLASH_QuadProgram(PAGE1_BASE_ADDRESS, sizeof(uint16_t), (uint8_t *)&value);
#else
        ...
        case RECEIVE_DATA:
            if (PageStatus1 == VALID_PAGE) { 
                ...
                value = VALID_PAGE;
#if (FLASH_OP_QUAL_MODE == ENABLE)
                FLASH_QuadProgram(PAGE0_BASE_ADDRESS, sizeof(uint16_t), (uint8_t *)&value);
#else
...
#if (FLASH_OP_QUAL_MODE == ENABLE)
        FLASH_QuadRead(Address, sizeof(uint32_t), (uint8_t *)&value);
#else
...
#if (FLASH_OP_QUAL_MODE == ENABLE)
        FLASH_QuadProgram(targetAddress, sizeof(uint32_t), (uint8_t *)&value);
#else
...
uint16_t EE_PageCopy(uint32_t srcPageStart, uint32_t destPageStart)
{
    ...
    while ((srcAddr > srcPageStart) && srcAddr < (srcDataEndAddr + 2)) {
        /* Get the current location content to be compared with virtual address */
#if (FLASH_OP_QUAL_MODE == ENABLE)
        FLASH_QuadRead(srcAddr, sizeof(uint32_t), (uint8_t *)&value);
#else
...
static uint16_t EE_PageTransfer(uint16_t VirtAddress, uint16_t Data)
{
    ...
    value = RECEIVE_DATA;
#if (FLASH_OP_QUAL_MODE == ENABLE)
    FLASH_QuadProgram(NewPageAddress, sizeof(uint16_t), (uint8_t *)&value);
#else
...
uint32_t EE_BinarySearch(uint32_t startAddr, uint32_t endAddr, uint32_t key)
{
    ...
    while (low <= high){
        mid = (low + high) / 2;
#if (FLASH_OP_QUAL_MODE == ENABLE)
        FLASH_Quad_Read(mid<<2, sizeof(uint32_t), (uint8_t *)&value);
#else
...
#if (FLASH_OP_QUAL_MODE == ENABLE)
            FLASH_QuadRead((mid - 1)<<2, sizeof(uint32_t), (uint8_t *)&value);
#else
